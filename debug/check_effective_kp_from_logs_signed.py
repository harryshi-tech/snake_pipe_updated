...
def main() -> None:
    ...
    ap.add_argument("--err_thr", type=float, default=0.5, help="use samples with |e_corr| > err_thr (rad)")
    ap.add_argument("--vel_thr", type=float, default=2.0, help="use samples with |dq| < vel_thr (rad/s)")
    ...
    for k, jname in enumerate(joints):
        ...
        ej = s * e[:, k]        # <-- corrected error
        dqj = dq[:, k]

        mask = (np.abs(ej) > args.err_thr) & (np.abs(dqj) < args.vel_thr)
        kp_eff = np.median(np.abs(tj[mask]) / (np.abs(ej[mask]) + 1e-12)) if np.any(mask) else float("nan")
        c = corr(tj[mask], ej[mask]) if np.any(mask) else float("nan")

        rows.append((jname, s,
                     float(np.percentile(np.abs(ej),95)), float(np.max(np.abs(ej))),
                     float(np.percentile(np.abs(tj),95)), float(np.max(np.abs(tj))),
                     kp_eff, c))

    rows.sort(key=lambda r: r[3], reverse=True)

    print("\n=== Effective Kp check (sign-corrected) ===")
    print(f"Run dir: {run_dir}")
    print(f"Sign map: {args.sign_map}")
    print(f"Using samples with |e_corr|>{args.err_thr} rad and |dq|<{args.vel_thr} rad/s")
